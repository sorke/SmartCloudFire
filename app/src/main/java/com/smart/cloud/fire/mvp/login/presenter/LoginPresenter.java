...
//    }
    ...
    public void login(final String User, final String Pwd, final Context context, final int type) {
//        String AppVersion = MyUtils.getBitProcessingVersion();
//        String userid = "+86-"+User;
//        MD5 md = new MD5();
//        String password = md.getMD5ofStr(Pwd);
        if(Pwd.length()==0){
            mvpView.getDataFail("请填写密码");
            return;
        }
        if(type==1){
            mvpView.showLoading();
        }
        addSubscription(apiStores1.login(User),new SubscriberCallBack<>(new ApiCallback<LoginModel>() {
            @Override
            public void onSuccess(LoginModel model) {
                if(model!=null){
                    int errorCode = model.getErrorCode();
                    if(errorCode==0){
                        editSharePereference(context,model,User,Pwd);
                        MyApp.app.setPrivilege(model.getPrivilege());
                        mvpView.getDataSuccess();
                    }else{
                        mvpView.autoLoginFail();
